...
### 📋 Booking Stream & SOP
Per-guest structured data view that mirrors the original PMS PDF layout in a responsive two-column format. **Left column** displays traces, booking notes, previous stays, and operational data. **Right column** shows facility bookings (venue-highlighted: green for Source, orange for Spice), allergies, and HK notes. Content-based column routing handles merged PDF text items, and a **4-step facility time reconstruction pipeline** recovers orphaned times: (1) cross-column recovery from Previous Stays rows, (2) standalone time-line merging, (3) multi-venue chain splitting, (4) orphaned leading-time reattachment. Built-in SOP modal (v25.0, 32 sections) covering every operational workflow from PDF upload to itinerary generation. Facilities column is always editable via inline textarea with dual-field sync (`facilitiesRaw` + `facilities`).

### 📋 Handover Hub
9-department digital handover system replacing paper shifts. Departments: **Housekeeping** (AM: rooms cleaned, DND/Plant-a-Tree mapping → PM: turndown with skip alerts), **Source** (AM: breakfast → PM: dinner + Google reviews), **Spice** (PM: dinner + AI reviews), **Reception** (check-ins, groups, VIPs, returns, dogs, tomorrow’s occupancy, Google reviews), **Spa** (treatments, feedback), **Maintenance** (jobs, parts, urgents), **Reservations** (bookings, amendments, future occupancy), **Night** (late arrivals, security, incidents), **Lake House** (AM: breakfast → PM: LH check-ins rooms 51–58, Google reviews). Features **auto-save** (1.5s debounce, no save button), **AM lock**, **Plant-a-Tree skip-turndown alerts**, and **AI Google review fetching**.

### 👥 Group Booking Detection
Automatic group detection from arrival data using three signals: (1) **surname matching** — 2+ rooms sharing the same surname, (2) **notes scanning** — “group booking”, “party of”, “X rooms” keywords, (3) **room cross-references** — “with room 7”, “linked room” mentions. Detected groups display as 👥 flag badges on guest cards and auto-populate the Reception Handover’s Group Bookings field with party name, room numbers, and guest names. Staff can manually add corporate groups.
